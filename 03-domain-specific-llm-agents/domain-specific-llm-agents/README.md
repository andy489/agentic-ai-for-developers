...
```
├── demo-1/            # General vs domain-specific LLM comparison
├── demo-2/            # Data preparation and preprocessing
├── demo-3/            # Model fine-tuning with LoRA (Mistral-7B)
├── demo-4/            # Privacy-preserving feedback loop and model updates
├── requirements.txt
├── env.example
└── README.md
```
...
- Python 3.8+
- CUDA-compatible GPU (recommended for demos 3 and 4)
- 16 GB+ RAM
...
Install dependencies per section (see each demo's README), or install everything at once:
...
python fine-tune-model.py           # step 3: LoRA update (requires demo-3 output)
...
- [SuperAdapters](https://github.com/AGI-Edgerunners/LLM-Adapters) for the advanced fine-tuning framework (demo-3/SuperAdapters)
