# Demo 4 — Privacy-preserving Feedback Loop and Continuous Learning

Implements a continuous learning pipeline: collect user feedback, anonymise PII, store it, and use it to further fine-tune the model from demo-3 via LoRA updates.
...
Loads the model from `./models/mistral7b_fitness_llm_finetuned/` (output of demo-3), applies LoRA, trains on the feedback dataset, and saves to `./models/mistral7b_fitness_llm_updated/`.

**Requires:**
- Completed demo-3 fine-tuning (`./models/mistral7b_fitness_llm_finetuned/` must exist)
...
|-----------|-------|-------|
| `r` | 16 | Higher rank than demo-3 for more update capacity |
...
│   └── feedback_dataset/
└── models/                      # Must be populated from demo-3
...
| `OSError: model not found` | Ensure demo-3 fine-tuning completed and model is at `./models/mistral7b_fitness_llm_finetuned/` |
